...
export default {
  ...
  methods: {
    ...
    getSpuBaseAttrs () {
      this.$http({
        url: this.$http.gulimalladornUrl(`/product/attr/base/listforspu/${this.spuId}`, 'gateway'),
        method: 'get'
      }).then(({data}) => {
    ...
    showBaseAttrs () {
      ...
      this.$http({
        url: this.$http.gulimalladornUrl(
          `/product/attrgroup/${this.catalogId}/withattr`, 'gateway'
        ),
        method: 'get',
    ...
    submitSpuAttrs () {
      ...
      this.$confirm('修改商品规格信息, 是否继续?', '提示', {
        ...
        .then(() => {
          this.$http({
            url: this.$http.gulimalladornUrl(`/product/attr/update/${this.spuId}`, 'gateway'),
            method: 'post',

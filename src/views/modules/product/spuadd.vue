...
export default {
  ...
  methods: {
    ...
    getMemberLevels () {
      this.$http({
        url: this.$http.gulimalladornUrl('/member/memberlevel/list', 'gateway'),
        method: 'get',
      ...
      if (!this.dataResp.steped[1]) {
        this.$http({
          url: this.$http.gulimalladornUrl(
            `/product/attr/sale/list/${this.spu.catalogId}`, 'gateway'
          ),
          method: 'get',
          params: this.$http.adornParams({
            page: 1,
    ...
    showBaseAttrs () {
      if (!this.dataResp.steped[0]) {
        this.$http({
          url: this.$http.gulimalladornUrl(
            `/product/attrgroup/${this.spu.catalogId}/withattr`, 'gateway'
          ),
          method: 'get',
          params: this.$http.adornParams({})
        }).then(({data}) => {

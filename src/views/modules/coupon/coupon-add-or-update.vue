...
export default {
  ...
  methods: {
    getMemberLevels() {
      //获取所有的会员等级
      this.$http({
        url: this.$http.gulimalladornUrl("/member/memberlevel/list"),
        method: "get",
        ...
        this.memberLevels = data.page.records;
      });
    },
    init(id) {
      ...
      this.$nextTick(() => {
        ...
        if (this.dataForm.id) {
          this.$http({
            url: this.$http.gulimalladornUrl(`/coupon/coupon/info/${this.dataForm.id}`),
            method: "get",
            params: this.$http.adornParams()
          }).then(({ data }) => {
            if (data && data.code === 0) {
